...
pub fn create_node_input() -> TreeNodeInput<Element> {
  TreeNodeInput(
    Element {
      ...
      content_renderer: Box::new(ClockElementRenderer),
...
#[derive(Clone)]
pub struct ClockElementRenderer;

impl ElementContentRenderer for ClockElementRenderer {
  fn render(&self, element: &Element, game: &Game) {
  ...
  fn measure(&self, _: &Element) -> Dimensions {
    Dimensions::square(CLOCK_ELEMENT_SIZE)
  }
